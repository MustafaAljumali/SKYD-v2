...
export default function App() {
  ...
  // Iraq center used as fallback coords when no farm boundary is drawn yet
  const FALLBACK_COORDS: [number, number] = [33.3152, 44.3661];

  // Fetch Live Weather — uses farm boundary centroid when available, Iraq center otherwise
  const fetchWeather = async () => {
    setWeatherSource('loading');
    setIsWeatherLoading(true);
    try {
      const [lat, lon] = savedGeoJSON ? getCentroid(savedGeoJSON) : FALLBACK_COORDS;
      ...
      case 'dashboard':
        ...
        return (
          <div className="space-y-6 text-black">
            ...
            {/* Setup nudge — shown only when no farm boundary is drawn yet */}
            {geoArea === 0 && (
              <div className="bg-amber-50 border border-amber-300 rounded-2xl px-5 py-4 flex items-center justify-between gap-4">
                ...
                <button
                  type="button"
                  onClick={() => setCurrentPage('geofence')}
                  className="shrink-0 px-3 py-1.5 bg-amber-500 hover:bg-amber-600 text-white text-xs font-black rounded-lg transition-colors cursor-pointer whitespace-nowrap"
                >
                  {isAr ? 'رسم الحدود' : 'Draw Boundary'}
                </button>
              </div>
            )}
