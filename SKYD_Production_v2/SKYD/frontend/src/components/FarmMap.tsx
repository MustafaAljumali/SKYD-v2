...
export function FarmMap({ isAr, onBoundaryChange, savedGeoJSON }: FarmMapProps) {
  ...
  useEffect(() => {
    ...
    const patchPolygonUnlimitedPoints = () => {
      ...
      // GUARD 4: _endPoint — force vertex addition when mc < MIN_VERTICES.
      // The original _endPoint has a proximity check: if the click is near
      // marker[0] it calls _finishShape INSTEAD of adding a vertex. GUARD 1
      // blocks _finishShape, but that leaves the click silently swallowed —
      // the user can't place a 4th point near the start of the polygon.
      // Fix: when we still need more vertices, call _addVertex directly and
      // skip the original close path entirely. Only delegate to origEndPoint
      // once we have enough vertices (closing is allowed there).
      const origEndPoint = polylineProto?._endPoint;
      if (origEndPoint) {
        LD.Polyline.prototype._endPoint = function (clientX: number, clientY: number, e: any) {
          const mc = this._markers?.length ?? 0;
          if (mc < MIN_VERTICES) {
            // Bypass origEndPoint's proximity-close path: add vertex directly.
            ...
            if (latlng) {
              try {
                ...
                origEndPoint.call(this, clientX, clientY, e);
              }
            }
            return;
          }
